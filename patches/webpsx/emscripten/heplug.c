...
extern int ems_request_file(const char *filename);
extern size_t get_file_size( FILE * f);													// Adapter.cpp
...
#define MAX_LIBS 16
static char requiredLibs[MAX_LIBS][MAX_LIB_NAME];
static int requiredLibCount;
static int psf_lib_meta(void * context, const char * name, const char * value)
{
	// Some PSFs reference more than two libraries (e.g. Wild Arms uses _lib, _lib2 and _lib3).
	// Capture all of them so he_init() can stage every one in the Emscripten VFS before the
	// actual psf_load() walk recurses into the lib chain.
	if ( !strncasecmp( name, "_lib", 4 ) )
	{
		int idx;
		if ( name[ 4 ] == '\0' )
		{
		...
		{
			idx = atoi( name + 4 ) - 1;  // "_lib2" -> 1, "_lib3" -> 2, ...
		}
		if ( idx >= 0 && idx < MAX_LIBS )
		{
			snprintf( requiredLibs[ idx ], MAX_LIB_NAME, "%s", value );
			if ( idx + 1 > requiredLibCount ) requiredLibCount = idx + 1;
			trace( "he: lib[%d]=%s\n", idx, value );
		}
	}
	return 0;
...
//		} else {
			...
			if (compressionEnabled && isCompressed(he_bios_path)) {
				size_t compressed_size = get_file_size( f );
				...
				size_t ps2_bios_size = get_file_size( f );
				if ( ps2_bios_size != 0x400000 ) {
					psf_file_fclose( f );
...
int he_init (DB_fileinfo_t *_info, const char * uri) {
 ...
 //   int psf_version = psf_load( uri, &psf_file_system, 0, 0, 0, 0, 0, 0 );
	requiredLibCount = 0;
	memset( requiredLibs, 0, sizeof( requiredLibs ) );
	...
	// Stage every referenced lib in the Emscripten VFS before the real load walk.
	// he_init is the only chance to ask JS to fetch a lib; once the second psf_load()
	// below recurses, the C side will fopen() the lib directly and the file must already
	// be present.
	{
		int i;
		for ( i = 0; i < requiredLibCount; i++ )
		{
			if ( !strlen( requiredLibs[ i ] ) ) continue;
			char tmpFileName[PATH_MAX];
			char *p = strrchr( uri, '/' );
			int pathlen = p ? ( p - uri + 1 ) : 0;
			memcpy( tmpFileName, uri, pathlen );
			snprintf( tmpFileName + pathlen, PATH_MAX - pathlen, "%s", requiredLibs[ i ] );

			trace( "he: requesting lib[%d] %s\n", i, tmpFileName );
			int r = ems_request_file( tmpFileName );	// trigger load & check if ready
			if ( r < 0 ) {
...
int he_read (DB_fileinfo_t *_info, short * samples, int size) {
