...
void psf_status_cb(void * context, const char * message) {
}
...
public:
	...
	int open(const char * p_path ) {
		...
		if ( psf_load( p_path, &psf_file_system, 0x21, 0, 0, psf_info_meta, &info_state, 0, psf_status_cb, 0) <= 0 ) {
...
restart:				
		...
		// NOTE: index-based iteration is required here. get_required_libs() returns
		// a fresh vector *by value*, and `libs` is reassigned inside the loop below.
		// Using an iterator would dangle (use-after-free) the moment `libs` is
		// reassigned, producing garbage lib names. info_set_lib() is append-only and
		// de-duplicated, so existing indices stay stable; growth triggers `restart`.
		for (size_t idx = 0; idx < libs.size(); ++idx) {
			const std::string libName= libs[idx];	// copy: `libs` may be reassigned below
			...
			if (r <0) { 
				...
				if ( psf_load( libFile.c_str(), &psf_file_system, 0x21, 0, 0, psf_info_meta, &info_state, 0, psf_status_cb, 0 ) <= 0 ) {	// used to parse potential additional _lib refs
					throw exception_io_data( "Not a USF file" );
				} else {
					libs= m_info.get_required_libs();
					
					if ((int)libs.size() > origLen)  {						
						goto restart;	// re-fetch list and rescan from the top
					}					
				}				
			}
		}
	...
	void decode_initialize(t_int16 *output_buffer, int out_size) {			
		...
		if ( psf_load( m_path.c_str(), &psf_file_system, 0x21, usf_loader, m_state, usf_info, m_state, 1, psf_status_cb, 0 ) < 0 )
